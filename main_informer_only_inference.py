'''
已经训练好模型存在checkpoints里了,只需要调用reference
'''
import argparse
import os
import torch
...
# parser.add_argument('--root_path', type=str, default='./data/SKAB/valve1/', help='root path of the data file')
...
for data_path in os.listdir(args.root_path):
    ...
    for ii in range(args.itr):
        ...
        setting = '{}_{}_ft{}_sl{}_ll{}_pl{}_dm{}_nh{}_el{}_dl{}_df{}_at{}_fc{}_eb{}_dt{}_mx{}_{}_{}_{}_{}'.format(args.model, args.data, args.features,
                    args.seq_len, args.label_len, args.pred_len,
                    args.d_model, args.n_heads, args.e_layers, args.d_layers, args.d_ff, args.attn, args.factor,
                    args.embed, args.distil, args.mix, args.des, ii, os.path.basename(args.data_path), data_path.replace('.', ''))

        exp = Exp(args) # set experiments

        print('>>>>>>>inference : {}<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<'.format(setting))
        exp.inference(setting)
